module.exports.run = async (bot, message, args) => {
  if (!message.guild.me.permissions.has("MANAGE_ROLES"))
    return message.reply("I Need Manage Roles Perm !");

  if (!message.member.permissions.has("MANAGE_ROLES"))
    return message.reply("You Need Manage Roles Perm !");

  if (!args[0])
    return message.reply(
      "**Usage : !addrole bots/humans/@role @user1 @user2**"
    );
  ...
  if (message.guild.me.roles.highest.position <= role.position)
    return message.reply("Pls Check my role is above that role ");

  if (
    !message.member.roles.highest.position >= role.position &&
    message.author.id != message.guild.ownerID
  )
    return message.reply("You Can't Manage That Role !");
  ...
  switch (args[0]) {
    case `bots`:
      ...
      await botsofserver.forEach((member) => {
        member.roles
          .add(role, `Responsible Moderator : ${message.author.tag}`)
    ...
    default:
      ...
      const msg1 = message.channel.send(
        `Adding Role \`${role.name}\` To \`${
          bandelog.size
        }\` Members \n Users => ${args.slice(1)}`
      );
      await bandelog.forEach((n) =>
        n.roles.add(role, `Responsible Moderator : ${message.author.tag}`)
      );
